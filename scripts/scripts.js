...
// readers correctly skip decorative images. Must stay byte-identical to
// dm-scene7-helpers.js EMPTY_ALT_SENTINEL.
...
function buildDynamicMediaImages(main) {
  ...
  main.querySelectorAll('a').forEach((a) => {
    ...
    // that point every DM anchor looks like a plain text link — no <img>
    // yet — so decorateButtons promotes it to a button and adds
    // `button-container` to its sole-child parent. Strip both classes
    // BEFORE rebuilding. Idempotent — no-op when the classes aren't present.
...
// optimizer in aem.js. Returning null for non-DM URLs lets the caller
// fall through to its standard path-only optimization.
